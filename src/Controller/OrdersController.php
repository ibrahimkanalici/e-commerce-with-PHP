...
class OrdersController extends AbstractController
{
     ...
     */
    public function new(Request $request, ShopcartRepository $shopcartRepository,CategoryRepository $categoryRepository): Response
    {
        $cats[0] = '<ul id = "menu-v"';
        $orders = new Orders();
        ...
        $shopcart2 = $shopcartRepository->getUserShopCart($userid);
        ...
        if($this->isCsrfTokenValid('form-order', $submittedToken))
        {
            if ($form->isSubmitted() ) {
                ...
                //Kredi Bilglerini servise gönder
                //Onay gelirse kaydetmeye devam et onay yoksa hata gönder
                $em = $this->getDoctrine()->getManager();

                $orders->setUserid($userid);
                $orders->setAmount($total);
                $orders->setStatus('Yeni');
                $em->persist($orders);
                $em->flush();
                ...
                foreach ($shopcart as $item)
                {
                    ...
                    $orderdetail->setStatus("Sipariş Verildi");
                ...
                WHERE s.userid=:userid')->setParameter('userid',$userid);
                $query->execute();
